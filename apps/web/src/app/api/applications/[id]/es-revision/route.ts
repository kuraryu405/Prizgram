...
import { EsAiService, revisionRequestSchema } from "@/server/es-ai/service";
...
export async function POST(
  ...
  return withNoStore(
    withApiHandler(async (innerRequest) => {
      ...
      const input = await readJsonBody(innerRequest, revisionRequestSchema);
      const result = await new EsAiService(getDatabase()).revise(
        user.id,
        id,
        input,
      );
