...
test("registers an active selection and adds its deadline", async ({ page }) => {
  await page.setExtraHTTPHeaders({ "cf-connecting-ip": "203.0.113.120" });
  const suffix = Math.random().toString(36).slice(2, 8);
  const loginId = `e2e.minimal.${Date.now()}-${suffix}`;
  ...
  const deadlineInput = page.getByLabel(/期限（Asia\/Tokyoの現地時刻）/);
  await deadlineInput.fill("2026-09-30T10:00");
